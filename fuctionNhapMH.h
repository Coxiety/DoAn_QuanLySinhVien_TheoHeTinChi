...
{
    if (root)
    {                      // Kiểm tra nếu root không phải là nullptr
...
bool checkTrungMaMH(PTRMH root, const string &maMH)
{
    ...
    if (maMH < root->mh.MaMH)
        return checkTrungMaMH(root->left, maMH);
...
void nhapMonHoc(PTRMH &root)
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            getline(cin, mh.MaMH);
            if (mh.MaMH.empty())
                return; // Kết thúc nếu người dùng nhập chuỗi rỗng
